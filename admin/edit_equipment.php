...
$page_title = "Edit Equipment - Admin";
$errors = [];
$form_data = []; // Initialize to avoid undefined errors if equipment not found
...
} elseif (isset($_POST['equipment_id']) && is_numeric($_POST['equipment_id'])) {
    $equipment_id_to_edit = (int)$_POST['equipment_id'];
}

if (!$equipment_id_to_edit && $_SERVER["REQUEST_METHOD"] !== "POST") {
    header("Location: manage_equipments.php?status=error&msg=" . urlencode("No equipment ID specified."));
    exit();
}

if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_equipment'])) {
    $form_data = $_POST; // Use POST data for re-population on error
    ...
    if (!empty($sku) && $equipment_id_to_edit && $pdo) {
        try {
            $stmt_check_sku = $pdo->prepare("SELECT equipment_id FROM equipments WHERE sku = :sku AND equipment_id != :current_equipment_id");
            $stmt_check_sku->bindParam(':sku', $sku);
            $stmt_check_sku->bindParam(':current_equipment_id', $equipment_id_to_edit, PDO::PARAM_INT);
            $stmt_check_sku->execute();
            if ($stmt_check_sku->fetch()) {
                $errors[] = "SKU already exists for another product. It must be unique.";
            }
        } catch (PDOException $e) {
            $errors[] = "Database SKU check failed: " . $e->getMessage();
        }
    }

    if (empty($errors) && $equipment_id_to_edit && $pdo) {
        try {
            ...
            $stmt->bindParam(':equipment_id', $equipment_id_to_edit, PDO::PARAM_INT);

            $stmt->execute();
            header("Location: manage_equipments.php?status=updated");
            exit();
        } catch (PDOException $e) {
            $errors[] = "Error updating equipment: " . $e->getMessage();
        }
    }
} elseif ($equipment_id_to_edit && $pdo) { // Fetch data for GET request or if POST failed (re-fetch original)
    if (empty($errors)) { // Only fetch from DB if there were no POST errors, otherwise $form_data from POST is used
        try {
            ...
            if (!$form_data) {
                header("Location: manage_equipments.php?status=error&msg=" . urlencode("Equipment not found."));
                exit();
            }
        } catch (PDOException $e) {
            $errors[] = "Error fetching equipment details: " . $e->getMessage();
            $form_data = [];
        }
    }
}
        ...
        <main class="admin-main-content-area">
            <div class="admin-content-container form-container">
                <div class="admin-header">
                    <h1 class="admin-page-title">Edit Equipment</h1>
                ...
                <?php if (!empty($form_data) && $equipment_id_to_edit): ?>
                    <form action="edit_equipment.php" method="POST" class="admin-form">
                        <input type="hidden" name="equipment_id" value="<?php echo htmlspecialchars($equipment_id_to_edit); ?>">
                        ...
                        <div class="form-group checkbox-group">
                            <input type="checkbox" id="is_featured" name="is_featured" value="1" <?php echo (isset($form_data['is_featured']) && $form_data['is_featured'] == 1) ? 'checked' : ''; ?>>
                            <label for="is_featured">Feature this equipment</label>
                        </div>
                        <div class="form-group">
                            <button type="submit" name="update_equipment" class="admin-button add-new-button">Update Equipment</button>
                        </div>
                    </form>
                <?php elseif (empty($errors)): ?>
                    <p>Equipment not found or no ID specified for editing.</p>
                <?php endif; ?>
            </div>
        </main>
    </div>
