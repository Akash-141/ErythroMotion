...
$page_title = "Edit Exercise - Admin";
$errors = [];
$form_data = []; 
$exercise_id_to_edit = null;
...
// Determine the exercise ID to edit
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $exercise_id_to_edit = (int)$_GET['id'];
} elseif (isset($_POST['exercise_id']) && is_numeric($_POST['exercise_id'])) { // If form submitted with ID
    $exercise_id_to_edit = (int)$_POST['exercise_id'];
}

if (!$exercise_id_to_edit && $_SERVER["REQUEST_METHOD"] !== "POST") { // If no ID and not a POST (initial load error)
    header("Location: manage_exercises.php?status=error&msg=" . urlencode("No exercise ID specified for editing."));
    exit();
}

// Handle form submission
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_exercise']) && $exercise_id_to_edit && $pdo) {
    $form_data = $_POST; // Use submitted data to repopulate form on error
    ...
    $is_featured = isset($form_data['is_featured_exercise']) ? 1 : 0; // Get 'is_featured' status
    ...
    if (empty($errors)) {
        try {
            ...
            $sql = "UPDATE exercises SET 
                        ...
                        body_part_targeted = :body_part_targeted, 
                    ...
                    WHERE exercise_id = :exercise_id"; // [cite: DDL.sql]
            ...
            $stmt->bindParam(':is_featured', $is_featured, PDO::PARAM_INT); // Bind new parameter
            $stmt->bindParam(':exercise_id', $exercise_id_to_edit, PDO::PARAM_INT);

            $stmt->execute();
            header("Location: manage_exercises.php?status=updated"); // [cite: manage_exercises.php]
            exit();

        } catch (PDOException $e) {
            $errors[] = "Error updating exercise: " . $e->getMessage();
        }
    }
    // If errors, $form_data is already populated from $_POST for repopulation.
    // We also need to ensure exercise_id is part of $form_data for hidden field if not already.
    $form_data['exercise_id'] = $exercise_id_to_edit;


} elseif ($exercise_id_to_edit && $pdo && empty($_POST)) { // Fetch data for GET request if not a POST with errors
    try {
        $stmt = $pdo->prepare("SELECT * FROM exercises WHERE exercise_id = :exercise_id"); // [cite: DDL.sql]
        $stmt->bindParam(':exercise_id', $exercise_id_to_edit, PDO::PARAM_INT);
        $stmt->execute();
        $form_data = $stmt->fetch(); // This will now include 'is_featured'
        if (!$form_data) {
            header("Location: manage_exercises.php?status=error&msg=" . urlencode("Exercise not found."));
            exit();
        }
        // The 'is_featured_exercise' for checkbox needs to be derived from 'is_featured' from DB
        $form_data['is_featured_exercise'] = $form_data['is_featured'] ?? 0;

    } catch (PDOException $e) {
        $errors[] = "Error fetching exercise details: " . $e->getMessage();
        $form_data = ['exercise_id' => $exercise_id_to_edit]; // Preserve ID at least
    }
}
?>
<!DOCTYPE html>
        ...
        <main class="admin-main-content-area">
            <div class="admin-content-container form-container">
                <div class="admin-header">
                    <h1 class="admin-page-title">Edit Exercise</h1>
                ...
                <?php if (!empty($form_data) && isset($form_data['exercise_id'])): // Ensure form_data is populated and has ID ?>
                <form action="edit_exercise.php?id=<?php echo htmlspecialchars($form_data['exercise_id']); ?>" method="POST" class="admin-form">
                    <input type="hidden" name="exercise_id" value="<?php echo htmlspecialchars($form_data['exercise_id']); ?>">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="difficulty_level" name="difficulty_level" required>
                                ...
                                <option value="<?php echo $level; ?>" <?php echo (isset($form_data['difficulty_level']) && $form_data['difficulty_level'] === $level) ? 'selected' : ''; ?>><?php echo $level; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select id="gender_target" name="gender_target" required>
                                ...
                                <option value="<?php echo $gender; ?>" <?php echo (isset($form_data['gender_target']) && $form_data['gender_target'] === $gender) ? 'selected' : ''; ?>><?php echo $gender; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <div class="form-group checkbox-group">
                        <input type="checkbox" id="equipment_needed" name="equipment_needed" value="1" <?php echo (isset($form_data['equipment_needed']) && $form_data['equipment_needed'] == 1) ? 'checked' : ''; ?>>
                    ...
                    <!-- New Checkbox for 'is_featured_exercise' -->
                    <div class="form-group checkbox-group">
                        <input type="checkbox" id="is_featured_exercise" name="is_featured_exercise" value="1" <?php echo (isset($form_data['is_featured_exercise']) && $form_data['is_featured_exercise'] == 1) ? 'checked' : ''; ?>>
                        <label for="is_featured_exercise">Feature this exercise on homepage</label>
                    </div>

                    <div class="form-group">
                        <button type="submit" name="update_exercise" class="admin-button add-new-button">Update Exercise</button>
                    </div>
                </form>
                <?php elseif (empty($errors)): // Only show this if no form data to display and no errors (e.g. initial error finding user) ?>
                     <p>Exercise not found or no ID specified for editing.</p>
                <?php endif; ?>
            </div>
        </main>
    </div>
